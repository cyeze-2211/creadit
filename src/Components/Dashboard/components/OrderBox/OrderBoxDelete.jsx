...
export default function BoxDelete({ product, onDelete, onCancel }) {
  ...
  // Modal tashqarisiga bosilganda yoki ESC bosilganda yopiladi
  useEffect(() => {
  ...
  // Animatsiya tugagandan keyin haqiqiy yopiladi
  useEffect(() => {
    if (closing) {
      const timer = setTimeout(() => {
        onCancel();
      }, 250); // fade-out davomiyligi bilan bir xil
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`/api/sells/${product.id}`, {
        headers: {
